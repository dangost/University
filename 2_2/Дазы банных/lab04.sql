...
select driverId from Report where dateOfExport between date_sub(now(),interval 1 week) and now();  # result if interval is 1 year
...
select driverId from Report where carId in (select carId from Cars where carNum in (select modelId from Models where Models.modelName = "Tesla Model A" or Models.modelName = "Tesla Model Y" or Models.modelName = "Tesla Model Z"));
...
select driverId from Report where dateOfExport not between date_sub(now(),interval 3 day) and now();
